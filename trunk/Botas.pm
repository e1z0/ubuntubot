...
use IO::Socket;
use POSIX;
use Ubuntubot::Parser;
use base qw(Ubuntubot::Config Ubuntubot::Actions Ubuntubot::Common Ubuntubot::Pipeline Ubuntubot::Util::Facts Ubuntubot::Util::Karma Ubuntubot::Util::UserAccess);
...
# start message
my $START_MSG = "UBUNTU BOT Starting...";
...
sub init
{
	...
	# processed lines
	$self->set(processed_lines => 0);
        $self->set(processed_questions => 0);
...
sub start_session
{
	...
	while (defined ( my $line = <$SOCK> ) )
	{
		...
		Ubuntubot::Parser::parse($self, $line);
